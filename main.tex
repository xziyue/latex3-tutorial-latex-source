...
In this material, we first briefly describe the difference between \LT{} and other programming languages.
Then, we introduce \LTT{}--a set of modern programming interfaces in \LT{} to help users write general purpose programs.
...
This material is an example based \LTT{} tutorial that introduces readers to the commonly used components of \LTT{}.
...
When being viewed as a programming language, \LT{} is significantly different from existing languages such as Python \cite{vanrossum2010python} and C \cite{ritchie1988c}.
% Fundamentally, \LT{} is a macro language that functions by replacing patterns according to a set of defined rules (known as macros).
% This replacement process is also known as \emph{macro expansion}.
In most programming languages, valid function/variable names and meanings of punctuations are predefined and cannot be changed.
...
In \LT{}, each character is associated with a hidden attribute known as the \emph{category code}.
There are 16 category codes in \LT{}, which are shown in \cref{tab:cat-code}.
...
\begin{table}[tb]
    ...
    \end{tabular}
    \caption{List of the 16 category codes in \LT{} and example characters with given category codes under standard \LT{} configuration.}
    \label{tab:cat-code}
\end{table}
...
% 15	Invalid character, not allowed to appear in the .tex input file	ASCII code 127 (\127)
